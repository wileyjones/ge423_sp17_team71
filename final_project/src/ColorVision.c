...
volatile float 	bobject_x 			= 0.0;
volatile float 	bobject_y 			= 0.0;
volatile float 	gobject_x 			= 0.0;
volatile float 	gobject_y 			= 0.0;
volatile int 	new_coordata 		= 0;
volatile int bnumpels = 0;
volatile int gnumpels = 0;
...
//            }
		...
		if (new_coordata == 0) {
			if (bgcolor==1){
				...
				if (final_object_stats[largest_object].num_pixels_in_object > 1) {
						noimagefound = 0;
						new_num_found_objects = num_unique_objects;
						gobject_x = cbar - IMAGE_COLUMNS/2;
						gobject_y = rbar - IMAGE_ROWS/2;
						gnumpels = final_object_stats[largest_object].num_pixels_in_object;
						//new_object_theta 	= final_object_stats[largest_object].theta;
						//new_C20				= final_object_stats[largest_object].C20_sum;
						//new_C02				= final_object_stats[largest_object].C02_sum;
						new_coordata = 1;
        ...
        if (final_object_stats[largest_object].num_pixels_in_object > 6) {
            ...
            ptrImage[rbar*IMAGE_COLUMNS+cbar].red = 255;
            ptrImage[rbar*IMAGE_COLUMNS+cbar].blue = 0;
            ptrImage[rbar*IMAGE_COLUMNS+cbar].green = 0;
            if (rbar > 0) {
                ptrImage[(rbar-1)*IMAGE_COLUMNS+cbar].red =  255;
                ptrImage[(rbar-1)*IMAGE_COLUMNS+cbar].blue = 0;
                ptrImage[(rbar-1)*IMAGE_COLUMNS+cbar].green = 0;
            }
            ...
            if (cbar > 0) {
                ptrImage[rbar*IMAGE_COLUMNS+(cbar-1)].red = 255;
                ptrImage[rbar*IMAGE_COLUMNS+(cbar-1)].blue = 0;
                ptrImage[rbar*IMAGE_COLUMNS+(cbar-1)].green = 0;
            }
